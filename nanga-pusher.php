...
class Nanga_Deploy_Pusher
{
    ...
    protected function _commands_for_database_import_thru_ssh(&$commands)
    {
        ...
        $commands[] = array(
            "ssh $ssh_db_user@$ssh_db_host -p $ssh_port \"cd $ssh_db_path; mysql --user=$db_user --password=$db_password --host=$db_host $db_name < db_to.sql; rm db_to.sql\"",
            true,
        );
        $commands[] = array('rm db_to.sql', true);
    }
    ...
    protected function _commands_for_database_import_locally(&$commands)
    {
        extract($this->params);
        $commands[] = array(
            "mysql --user=$db_user --password=$db_password --host=$db_host $db_name < db_to.sql;",
            true,
        );
        $commands[] = array('rm db_to.sql', true);
    }
    ...
    protected function _commands_for_database_dump(&$commands)
    {
        ...
        $commands = array(
            array(
                'wp migratedb export db_from.sql --exclude-post-revisions --skip-replace-guids --include-transients',
                true,
            ),
        );
